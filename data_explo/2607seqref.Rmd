...
tax_final<-readRDS("tax_euka2207final.rds")
...
create.fasta <- function(rank, x){  #rank is the level of classification we want as the nb of column, x is the specific taxon we want in this level
...
compar.pres<-function(x){  #this function creates a table with every sequences from a genus, its invented unique name, and the nb of samples it appears in
  ...
  res<-as.data.frame(create.fasta(7,x)) #we get that table with all sequences from a genus+names given
  print(res)
  ...
  for (l in 1:length(seqs)){ #go through all sequences
    ...
    for (i in 1:length(namesbin)){ #go through the whole table
      ...
      nbpres<-c(nbpres, sum(bin_final[i,])) #count the presence and had the value to the vector
        break
      }
...
for (g in 1: length(distlist)){
  ...
  #Identifying sequences present in enough samples
  mainseq<-c()
...
plot(shindex,as.numeric(aggpatho2[7,])) ####CORRELATION NEGATIVE!!!
